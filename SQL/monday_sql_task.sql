...
use RTO1;
CREATE TABLE orders (id INT PRIMARY KEY,order_name VARCHAR(255),cost DECIMAL(10, 2),created_at TIMESTAMP,created_by VARCHAR(255),modified_at TIMESTAMP,modified_by VARCHAR(255));
INSERT INTO orders (id, order_name, cost, created_at, created_by, modified_at, modified_by) 
VALUES
...
CREATE TABLE payments (id INT PRIMARY KEY,price DECIMAL(10, 2),order_id INT,payment_status VARCHAR(50),created_at TIMESTAMP,created_by VARCHAR(255),modified_at TIMESTAMP,modified_by VARCHAR(255),FOREIGN KEY (order_id) REFERENCES orders(id));
INSERT INTO payments (id, price, order_id, payment_status, created_at, created_by, modified_at, modified_by) 
VALUES
...
CREATE TABLE restaurants (id INT PRIMARY KEY,restaurant_name VARCHAR(255),loc VARCHAR(255),order_id INT,ratings INT,payment_id INT,created_at TIMESTAMP,created_by VARCHAR(255),modified_at TIMESTAMP,modified_by VARCHAR(255),FOREIGN KEY (order_id) REFERENCES orders(id),FOREIGN KEY (payment_id) REFERENCES payments(id));
INSERT INTO restaurants (id, restaurant_name, loc, order_id, ratings, payment_id, created_at, created_by, modified_at, modified_by)
 VALUES
...
CREATE TABLE delivery (id INT PRIMARY KEY,person_name VARCHAR(255),mode_of_payment 
ENUM('Cash', 'Card', 'Online'),restaurant_id INT,order_id INT,created_at TIMESTAMP
,created_by VARCHAR(255),modified_at TIMESTAMP,modified_by VARCHAR(255),
FOREIGN KEY (restaurant_id) REFERENCES restaurants(id),FOREIGN KEY (order_id) REFERENCES orders(id));
...
desc delivery;
INSERT INTO delivery VALUES
(11, 'DeliveryPerson4', 'Cash', 4, 4, '2023-07-04 15:00:00', 'User4', '2023-07-04 16:00:00', 'User5');